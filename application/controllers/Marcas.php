...
//        exit();
    ...
    public function add() {
        ...
        $this->form_validation->set_rules('marca_nome', 'nome', 'trim|required|min_length[2]|max_length[45]|is_unique[marcas.marca_nome]');
    ...
    public function edit($marca_id = NULL) {
        ...
        if(!$marca_id || !$this->core_model->get_by_id('marcas', array('marca_id' => $marca_id))) {
            ...
            $this->form_validation->set_rules('marca_nome', 'nome', 'trim|required|min_length[2]|max_length[45]');
            ...
            if($this->form_validation->run()) {
                ...
                if($this->db->table_exists('produtos')) {
                    ...
                    if($marca_ativa == 0 && $this->core_model->get_by_id('produtos', array('produto_marca_id' => $marca_id, 'produto_ativo' => 1))) {
                        ...
                        $this->session->set_flashdata('error', 'Esta marca não pode ser desativada, pois está sendo utilizada em <i class="fab fa-product-hunt"></i>&nbsp;Produtos');
                        redirect('marcas');
                    }
                }
