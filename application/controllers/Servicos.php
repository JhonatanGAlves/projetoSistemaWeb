...
//        exit();
    ...
    public function add() {
        ...
        $this->form_validation->set_rules('servico_nome', 'nome', 'trim|required|min_length[10]|max_length[145]|is_unique[servicos.servico_nome]');
        $this->form_validation->set_rules('servico_preco', 'preço', 'trim|required');
        $this->form_validation->set_rules('servico_descricao', 'descrição', 'trim|required|max_length[700]');
    ...
    public function edit($servico_id = NULL) {
        ...
        if(!$servico_id || !$this->core_model->get_by_id('servicos', array('servico_id' => $servico_id))) {
            ...
            $this->form_validation->set_rules('servico_nome', 'nome', 'trim|required|min_length[10]|max_length[145]|callback_check_nome_servico');
            $this->form_validation->set_rules('servico_preco', 'preço', 'trim|required');
            $this->form_validation->set_rules('servico_descricao', 'descrição', 'trim|required|max_length[700]');
